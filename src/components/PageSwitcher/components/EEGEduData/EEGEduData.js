...
import { TextContainer, Card, Stack, RangeSlider, Button, ButtonGroup, Modal, TextField } from "@shopify/polaris";
...
export function buildPipe(Settings) {
  ...
  window.pipeData$ = null;
  window.multicastData$ = null;
  window.subscriptionData = null;

  // Build Pipe 
  window.pipeData$ = zipSamples(window.source.eegReadings$).pipe(
    bandpassFilter({ 
  ...
  window.multicastData$ = window.pipeData$.pipe(
...
export function setup(setData, Settings) {
  ...
  if (window.multicastData$) {
    window.subscriptionData = window.multicastData$.subscribe(data => {
      setData(alphaData => {
...
function saveToCSV(Settings, condition) {
  ...
  localObservable$ = window.multicastData$.pipe(
    take(1)
  );
